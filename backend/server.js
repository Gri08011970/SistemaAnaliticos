...
app.post("/api/matricula/importar", async (req, res) => {
  try {
    ...
    const alumnosGuardados =
      await MatriculaAlumno.insertMany(alumnosConRecursante);
...
app.get("/api/seguimiento", async (req, res) => {
  try {
    const { curso, asignatura, alumnoId, periodo } = req.query;
    ...
    console.error("Error al obtener seguimiento pedagógico:", error);

    res.status(500).json({
      mensaje: "Error al obtener los registros de seguimiento pedagógico",
    });
  }
});
...
app.put("/api/seguimiento", async (req, res) => {
  try {
    ...
    if (!alumnoId || !curso || !asignatura || !periodo) {
      return res.status(400).json({
        mensaje: "Faltan alumnoId, curso, asignatura o período",
      });
    }

    const registroActualizado = await SeguimientoPedagogico.findOneAndUpdate(
      {
    ...
    console.error("Error al guardar seguimiento pedagógico:", error);

    res.status(500).json({
      mensaje: "Error al guardar el registro de seguimiento pedagógico",
    });
  }
});
...
app.post("/api/seguimiento/importar", async (req, res) => {
  try {
    ...
    if (operaciones.length === 0) {
      return res.status(400).json({
        mensaje: "Los registros recibidos no contienen los datos necesarios",
      });
    }

    const resultado = await SeguimientoPedagogico.bulkWrite(operaciones);

    res.json({
      mensaje: "Seguimiento pedagógico importado correctamente",
    ...
    console.error("Error al importar seguimiento pedagógico:", error);

    res.status(500).json({
      mensaje: "Error al importar los registros de seguimiento pedagógico",
    });
  }
});
...
app.delete("/api/seguimiento/:id", async (req, res) => {
  try {
    const registro = await SeguimientoPedagogico.findByIdAndDelete(
      req.params.id,
    );
    ...
    res.json({
      mensaje: "Registro de seguimiento eliminado correctamente",
    });
  } catch (error) {
    console.error("Error al eliminar seguimiento pedagógico:", error);
